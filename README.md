...
(Adidas só no Clube Netshoes). Varre 8 fontes, guarda histórico de preços em
...
| `netshoes_adidas_originals` | `clube.netshoes.com.br/busca?marca=adidas-originals` | Mesma estratégia da fonte `netshoes`, filtrando por Adidas Originals | ✅ |
| `approve` | `approve.com.br` | HTML Tiendanube + parse de listagem | ✅ |
...
- `ADMIN_TOKEN` — protege a rota auxiliar `/setup-webhook`
...
**Centauro e Approve não são executadas no Actions por padrão.** Centauro sofre
bloqueio agressivo do Akamai em IPs de runner, e o workflow atual prioriza as
fontes mais estáveis: `ous netshoes baw netshoes_baw netshoes_adidas
netshoes_adidas_originals`. Para incluir Centauro, rode local com proxy ou use
o bot on-demand no servidor.
